...
function load_files() {
  ...
  fs.readdir(dir, function(err, files) {
    ...
    if(err || _.isUndefined(files)) {
      def.reject('Error reading /seeds folder');
    } else {
...
function collectionExist(db, name) {
  ...
  return q.promise(function(resolve, reject, notify) {
    ...
    db.collectionNames(function(err, collections) {
      ...
      if(_.findWhere(collections, {name: (db.databaseName + '.' + name)})) {
        return resolve(true);
      } else {
...
function insert(db, collection_name, contents) {
  ...
  return q.promise(function(resolve, reject, notify) {
    ...
    if(contents.length !== 0) {
      ...
      db.collection(collection_name).insert(EJSON.parse(contents), function(err, result) {
